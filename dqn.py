...
class Net(nn.Module):
    def __init__(self, n_states, n_actions):
        ...
        self.fc1 = nn.Linear(
            n_states, 8
        )  # linear input layer with N_STATES number of nodes
        ...
        self.out = nn.Linear(
            8, n_actions
        )  # output layer with N_ACTIONS number of nodes
...
class DQN(object):
    def __init__(self, n_states, n_actions):
        ...
        self.eval_net, self.target_net = Net(self.n_states, self.n_actions).to(
            device
        ), Net(self.n_states, self.n_actions).to(device)

        self.learn_step_counter = 0  # for target updating
        self.memory_counter = 0  # for storing memory
        self.memory = np.zeros(
            (MEMORY_CAPACITY, self.n_states * 2 + 2)
        )  # initialize memory
    ...
    def learn(self):
        ...
        b_r = torch.FloatTensor(b_memory[:, self.n_states + 1 : self.n_states + 2]).to(
            device
        )
    ...
    def load(self, path):
        ...
        self.target_net = torch.load(
            path + "/target_net.pt", map_location=torch.device("cpu")
        ).to(device)
        self.eval_net.eval()
